...
public class ApexValidator : BaseValidator
{
    ...
    private PollResponseMessage? SendPollMessage()
    {
        ...
        var pollResponse = new PollResponseMessage(deviceData);
        ...
        if (!pollResponse.IsValid)
        {
            ...
            if (pollResponse.HasProtocolViolation && Config.StrictMode)
            {
                ...
                Logger.Error("{0} Problems: {1}", GetType().Name,
                    string.Join(Environment.NewLine, pollResponse.GetPayloadIssues()));
                return null;
            }
        ...
        if (nextMessage.IsAckNumberOne != pollResponse.IsAckNumberOne)
        {
    ...
    private void HandleEscrow(PollResponseMessage pollResponse)
    {
    ...
    /// </summary>
    /// <param name="pollResponse">Poll response from device</param>
    private void HandleState(PollResponseMessage pollResponse)
    {
    ...
    /// </summary>
    /// <param name="pollResponse">Poll response from device</param>
    private void HandleEvents(PollResponseMessage pollResponse)
    {
    ...
    /// </summary>
    /// <param name="pollResponse">Poll response from device</param>
    private void HandleCashBox(PollResponseMessage pollResponse)
    {
    ...
    /// </summary>
    /// <param name="pollResponse">Poll response from device</param>
    private void HandleCredit(PollResponseMessage pollResponse)
    {
    ...
    {
        var nextMessage = new PollRequestMessage(_apexState.Ack)
